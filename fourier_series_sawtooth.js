...
let s; // Receberá a soma da série
...
function graphFunction(){
  ...
  stroke(255,0,0);
  line(-L * graphScale,-L * graphScale,0,0);
  line(0,0,L * graphScale,-L * graphScale);
}
...
function graphSeries(){
  ...
  for(let x = -axisXSize; x <= axisXSize; x+= 0.01){
    ...
    for(let n=1 ; n<=a ; n++){ // Soma
      s += (((cos(n * (PI/L) * x)) * ((2 * pow(-1,n)) - 2)) / ((pow(PI,2)/L) * pow(n,2)))
    }

    vertex(x * graphScale,s * (-1) * graphScale); // Desenha a função
